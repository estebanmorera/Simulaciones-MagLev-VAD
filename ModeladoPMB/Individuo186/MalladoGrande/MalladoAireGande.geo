...
domain_radius = 22.0e-3;
domain_height = 26.0e-3;
...
AirDom[] = BooleanDifference{ Volume{100}; Delete; }{ Volume{MagExt1[0], MagExt2[0], MagInt1[0], MagInt2[0]}; Delete; };

// Coherencia final
Coherence;

// ------------------ DEFINICIÓN DE GRUPOS FÍSICOS ------------------
Physical Volume("Air_Domain") = {AirDom[0]};
Physical Volume("Magnet_Exterior_1") = {MagExt1[0]};
Physical Volume("Magnet_Exterior_2") = {MagExt2[0]};
Physical Volume("Magnet_Interior_1") = {MagInt1[0]};
Physical Volume("Magnet_Interior_2") = {MagInt2[0]};
...
// --- tamaños de malla por volumen ---
lc_air = 1.2e-3;   // malla más gruesa en el cilindro de aire
lc_mag = 2.5e-4;   // malla más fina en los imanes

// aplica el tamaño a los puntos de cada volumen
MeshSize { PointsOf{ Volume{AirDom[]}; } } = lc_air;
MeshSize { PointsOf{ Volume{MagExt1[], MagExt2[], MagInt1[], MagInt2[]}; } } = lc_mag;
...
// ------------------ MALLA ------------------
Mesh.CharacteristicLengthMin = min(lc_mag, lc_air);
Mesh.CharacteristicLengthMax = max(lc_mag, lc_air);
